...
公式ベンチのコードを読むと、JWTは作成から30秒で期限切れになります。さらにログにはmainが応答しないtimeoutが出ていました。認証の仕様を緩めて期限切れJWTを通せば、見かけの点は守れます。でもそれはボトルネックを隠す変更です。
...
同じreboot後、コードも設定も変えずに再試行すると`3,072,854 / deduction 0`でpassしました。だから最初の0点を消してよい、とは考えません。
